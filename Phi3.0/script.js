"use strict";
const _i = ['Phigros 模拟器', [3, 114, 514], 1611795955, 1634971621];
	...
	if (this.value == "播放") {
		...
		for (const i of chartLineData) {
			if (selectchart.value == i.Chart) {
				...
				Renderer.chart.judgeLineList[i.LineId].images[1] = await createImageBitmap(imgShader(bgs[i.Image], "#ffffff"));
				Renderer.chart.judgeLineList[i.LineId].images[2] = await createImageBitmap(imgShader(bgs[i.Image], "#ffffff"));
...
function loop() {
	...
	//Copyright
	ctx.font = `${lineScale * 0.4}px Lekton04`;
	ctx.fillStyle = "#ccc";
	ctx.globalAlpha = 0.8;
	ctx.textAlign = "right";
	ctx.textBaseline = "middle";
	ctx.fillText(`       `, (canvas.width + canvasos.width) / 2 - lineScale * 0.1, canvas.height - lineScale * 0.2);
...
function qwqdraw1(now) {
	...
	for (const i of clickEvents1) { //绘制打击特效1
		...
		ctxos.globalAlpha = 1;
		ctxos.setTransform(noteScale * 9, 0, 0, noteScale * 9, i.offsetX, i.offsetY); //缩放
		...
		const r3 = 0 * (((0.2078 * tick - 1.6524) * tick + 1.6399) * tick + 0.4988); //方块大小
	...
	if (qwqIn.second >= 3 && qwqOut.second == 0) {
		if (showPoint.checked) { //绘制定位点
			ctxos.font = `${lineScale}px Lekton04`;
	...
	if (qwqIn.second < 3) {
		...
		ctxos.drawImage(res["JudgeLine"], -imgW / 2, -imgH / 2, imgW, imgH);
	}
	//绘制分数和combo以及暂停按钮
	ctxos.textAlign = "center";
	ctxos.globalAlpha = 1;
	...
	ctxos.textBaseline = "alphabetic";
	ctxos.font = `${lineScale * 1.25}px Lekton04`;
	ctxos.fillText(stat.scoreStr, wlen, lineScale * 1.5);
	if (!qwq[0]) ctxos.drawImage(res["Pause"], lineScale * 0.8, lineScale * 0.8, lineScale * 0.8, lineScale * 0.8);
	{
		ctxos.textAlign = "center";
		ctxos.fillStyle = "#00a8f3";
		ctxos.font = `${lineScale * 1.1}px Lekton04`;
		ctxos.fillText(stat.combo, wlen, lineScale * 2.775);
		ctxos.globalAlpha = qwqIn.second < 0.67 ? tween[2](qwqIn.second * 1.5) : (1 - tween[2](qwqOut.second * 1.5));
		ctxos.font = `${lineScale * 0.4}px Lekton04`;
		ctxos.fillStyle = "#ffffff";
		ctxos.fillText(autoplay.checked ? "AUTOPLAY" : "COMBO", wlen, lineScale * 3.2);
	}
	//不知名蓝框
	ctxos.drawImage(res["top"], canvasos.width - canvasos.width / 2, canvasos.height - lineScale * 19.5, lineScale * 8, lineScale * 5);
	//绘制歌名和等级
	ctxos.globalAlpha = 1;
	ctxos.setTransform(1, 0, 0, 1, 0, lineScale * (qwqIn.second < 0.67 ? (1 - tween[2](qwqIn.second * 1.5)) : tween[2](qwqOut.second * 1.5)) * 1.75);
	ctxos.textBaseline = "alphabetic";
	ctxos.textAlign = "right";
	ctxos.font = `${lineScale * 0.63}px Lekton04`;
	ctxos.fillText(stat.accStr, canvasos.width - lineScale * 0.75, canvasos.height - lineScale * 0.66);
	ctxos.drawImage(res["SongsNameBar"], lineScale * 0, canvasos.height - lineScale * 0, lineScale * 0, lineScale * 0);
	ctxos.textAlign = "left";
	ctxos.font = `${lineScale * 0.63}px Lekton`;
...
function qwqdraw3(statData) {
	...
	ctxos.drawImage(res["LevelOver4"], -300, 825, 1184, 228);
	ctxos.globalAlpha = range((qwqEnd.second - 0.27) / 0.83);
	ctxos.drawImage(res["LevelOver1"], 102, 378);
	ctxos.globalCompositeOperation = "source-over";
	ctxos.globalAlpha = 1;
	ctxos.drawImage(res["LevelOver5"], 700 * tween[8](range(qwqEnd.second * 1.25)) - 600, 895, 20, 80);
	//歌名和等级
	ctxos.fillStyle = "#fff";
	ctxos.textBaseline = "middle";
	ctxos.textAlign = "left";
	ctxos.font = "70px Lekton04";
	ctxos.fillText(inputName.value || inputName.placeholder, 700 * tween[8](range(qwqEnd.second * 1.25)) - 550, 950);
	ctxos.font = "30px Lekton04";
	ctxos.fillText(inputLevel.value || inputLevel.placeholder, 700 * tween[8](range(qwqEnd.second * 1.25)) - 10, 975);
	//Rank图标
	...
	if (statData[3]) {
		...
		ctxos.fillText("Autoplay", 1355, 590);
