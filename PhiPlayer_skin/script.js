"use strict";
const _i = ['PhiPlayer skin', [1, 0, 0], 1611795955, 1652747710];
const urls = {
...
function qwqdraw1(now) {
	...
	if (document.getElementById("showCE2").checked)
		for (const i of clickEvents2) { //绘制打击特效2
			...
			ctxos.font = `bold ${noteScale*(256+128* (((0.2078 * tick - 1.6524) * tick + 1.6399) * tick + 0.4988))}px PhigrimSans`;
	...
	for (const i of clickEvents1) { //绘制打击特效1
		const tick = (now - i.time) / i.duration;
		ctxos.globalAlpha = 1;
		ctxos.setTransform(...imgFlip(noteScale * 7, 0, 0, noteScale * 7, i.offsetX, i.offsetY)); //缩放
	...
	if (qwqIn.second >= 3 && qwqOut.second == 0) {
		if (showPoint.checked) { //绘制定位点
			ctxos.font = `${lineScale * 0.8}px PhigrimSans`;
			ctxos.textAlign = "center";
	...
	if (qwqIn.second < 3) {
		...
		ctxos.font = `${lineScale * 1.1}px PhigrimSans`;
		const dxsnm = ctxos.measureText(inputName.value || inputName.placeholder).width;
		if (dxsnm > canvasos.width - lineScale * 1.5) ctxos.font = `${(lineScale) * 1.1/dxsnm*(canvasos.width-lineScale*1.5)}px PhigrimSans`;
		ctxos.fillText(inputName.value || inputName.placeholder, wlen, hlen * 0.75);
		//曲绘和谱师
		ctxos.textBaseline = "top";
		ctxos.font = `${lineScale * 0.55}px PhigrimSans`;
		const dxi = ctxos.measureText(`Illustration designed by ${inputIllustrator.value || inputIllustrator.placeholder}`).width;
		if (dxi > canvasos.width - lineScale * 1.5) ctxos.font = `${(lineScale) * 0.55/dxi*(canvasos.width-lineScale*1.5)}px PhigrimSans`;
		...
		if (dxc > canvasos.width - lineScale * 1.5) ctxos.font = `${(lineScale) * 0.55/dxc*(canvasos.width-lineScale*1.5)}px PhigrimSans`;
	...
	ctxos.textBaseline = "alphabetic";
	ctxos.font = `${lineScale * 0.95}px PhigrimSans`;
	ctxos.textAlign = "right";
	ctxos.fillText(stat.scoreStr, canvasos.width - lineScale * 0.65, lineScale * 1.375);
	ctxos.drawImage(res["Pause"], lineScale * 1, lineScale * 0.75, lineScale * 1, lineScale * 1);
	ctxos.font = `${lineScale * 0.7}px PhigrimSans`;
	ctxos.fillText(stat.accStr, canvasos.width - lineScale * 0.75, lineScale * 2.25);
	if (stat.combo > 2) {
		ctxos.textAlign = "center";
		ctxos.font = `${lineScale * 1.32}px PhigrimSans`;
		...
		ctxos.font = `${lineScale * 0.53}px PhigrimSans`;
	...
	ctxos.textBaseline = "alphabetic";
	ctxos.textAlign = "right";
	ctxos.font = `${lineScale * 0.63}px PhigrimSans`;
	const dxlvl = ctxos.measureText(inputLevel.value || inputLevel.placeholder).width;
	if (dxlvl > wlen - lineScale) ctxos.font = `${(lineScale) * 0.63/dxlvl*(wlen - lineScale )}px PhigrimSans`;
	ctxos.fillText(inputLevel.value || inputLevel.placeholder, canvasos.width - lineScale * 0.75, canvasos.height - lineScale * 0.66);
	ctxos.textAlign = "left";
	ctxos.font = `${lineScale * 0.63}px PhigrimSans`;
	const dxsnm = ctxos.measureText(inputName.value || inputName.placeholder).width;
	if (dxsnm > wlen - lineScale) ctxos.font = `${(lineScale) * 0.63/dxsnm*(wlen - lineScale )}px PhigrimSans`;
	...
	if (qwq[0]) {
		...
		ctxos.textBaseline = "middle";
		ctxos.font = `${lineScale * 0.25}px PhigrimSans`;
		ctxos.textAlign = "right";
		ctxos.textBaseline = "alphabetic";
...
function qwqdraw3(statData) {
	...
	ctxos.textBaseline = "middle";
	ctxos.textAlign = "left";
	ctxos.font = "80px PhigrimSans";
	const dxsnm = ctxos.measureText(inputName.value || inputName.placeholder).width;
	if (dxsnm > 1500) ctxos.font = `${80/dxsnm*1500}px PhigrimSans`;
	ctxos.fillText(inputName.value || inputName.placeholder, 700 * tween[8](range(qwqEnd.second * 1.25)) - 320, 145);
	ctxos.font = "30px PhigrimSans";
	const dxlvl = ctxos.measureText(inputLevel.value || inputLevel.placeholder).width;
	if (dxlvl > 750) ctxos.font = `${30/dxlvl*750}px PhigrimSans`;
